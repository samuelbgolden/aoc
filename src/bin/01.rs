...
pub fn part_one(input: &str) -> Option<u32> {
    ...
    input.split('\n').for_each(|line| {
        ...
        list1.push(vals.get(0).expect("l1 has val").parse::<i64>().expect("l1 parse val"));
        list2.push(vals.get(1).expect("l2 has val").parse::<i64>().expect("l2 parse val"));
    });
...
pub fn part_two(input: &str) -> Option<u32> {
    ...
    input.split('\n').for_each(|line| {
        ...
        if vals.len() != 2 { return }
        list1.push(vals.get(0).expect("l1 has val").parse::<i64>().expect("l1 parse val"));
        *freqs.entry(vals.get(1).expect("l2 has val").parse::<i64>().expect("l2 parse val")).or_insert(0) += 1;
    });
